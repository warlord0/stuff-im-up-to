...
We know about the environment variables from [Proxy Fun and Games](/posts/proxy-fun-and-games/) and [Sudo and Proxy / Environment Variables](/posts/sudo-and-proxy-environment-settings/). But if we hard code these variables into `/etc/environment` then what happens when we're on the road, out of the office? We still get a proxy that we need to turn off.
...
Now this does cause a problem with `apt`. But that's only because you need to follow the above article on [Sudo and Proxy / Environment Variables](/posts/sudo-and-proxy-environment-settings/).
